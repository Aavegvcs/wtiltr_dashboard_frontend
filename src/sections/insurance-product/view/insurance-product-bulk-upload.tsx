...
const ProductBulkUploadModal = ({ open, onClose, refreshData, insuranceCompany }: any) => {
  ...
  const handleDownloadSample = () => {
    ...
    link.href = url;
    link.setAttribute('download', 'sample_product_template.csv');
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < jsonData.length; i += batchSize) {
      ...
      try {
        const response = await axiosInstance.post('insurance-product/productBulkUpload', {
          data: batch,
          startIndex: i + 1,
      ...
      >
        ...
        <Box mb={1}>
          <Typography variant="body1" sx={{ mb: 0.5 }}>
            Insurance Company <RequiredMark />
          </Typography>
