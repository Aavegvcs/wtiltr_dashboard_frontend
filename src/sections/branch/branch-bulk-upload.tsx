...
export default function BranchBulkUpload({ open, onClose, refresh }: any) {
  ...
  const handleDownloadFailed = () => {
    ...
    const headers = ['Index', 'Branch Name', 'Reason'];
    const rows = failedRecords.map((rec) => [
      rec.index,
      rec.name ?? 'N/A',
      rec.reason ?? 'Unknown Error',
    ...
    try {
      ...
      for (let i = 0; i < jsonData.length; i += batchSize) {
        ...
        try {
          ...
          failed.push({
            index: i + 1,
            name: batch?.[0]?.Name ?? 'Unknown',
            reason: 'Server error while uploading batch',
          });
        }

        setProgress(Math.min(100, Math.round(((i + batchSize) / jsonData.length) * 100)));
      }
      ...
      if (failed.length === 0) {
        ...
        toast.success('Bulk upload completed with some failures');
      }
    } catch (error) {
      console.error(error);
      toast.error('Failed to process Excel file');
